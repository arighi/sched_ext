...
static bool sched_stress_test(void)
{
	...
	/* Get runtime for the CFS task */
	cfs_runtime = get_process_runtime(cfs_pid);
	if (cfs_runtime != -1)
		ksft_print_msg("Runtime of CFS task (PID %d) is %f seconds\n", cfs_pid, cfs_runtime);
	else
		ksft_exit_fail_msg("Error getting runtime for CFS task (PID %d)\n", cfs_pid);
	...
	ksft_print_msg("CFS task got %.2f%% of total runtime\n", actual_ratio * 100);

	if (actual_ratio >= expected_min_ratio) {
		ksft_test_result_pass("PASS: CFS task got more than %.2f%% of runtime\n",
				      expected_min_ratio * 100);
		return true;
	} else {
		ksft_test_result_fail("FAIL: CFS task got less than %.2f%% of runtime\n",
				      expected_min_ratio * 100);
		return false;
	}
}
